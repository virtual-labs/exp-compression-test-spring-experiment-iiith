...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function play() {
    ...
    document.getElementById("result").innerHTML =
      '<p class="v-bold">Status: Running...</p>';
  ...
  function pause() {
    window.clearTimeout(tmHandle);
    document.getElementById("result").innerHTML =
      '<p class="v-bold">Status: Paused</p>';
    pauseButton.setAttribute("disabled", "true");
    playButton.removeAttribute("disabled");
  }
  ...
  function drawStatic() {
    ...
    for (let i = 0; ; i++) {
      const start =
        midX + params["widthString"] * Math.sin(((i * Math.PI) / 72) * ratio);
  ...
  function draw() {
    drawStatic();
    if (ratio <= -2 && direction !== 0) {
      direction = 0;
      halfCycleCount++;
    } else if (ratio >= -1 && direction !== 1) {
      direction = 1;
      halfCycleCount++;
    }

    if (halfCycleCount >= maxHalfCycles) {
      simulationDone = true;
      document.getElementById("result").innerHTML =
        '<p class="v-bold">Status: Simulation complete</p><p>Use Restart to reset to initial state.</p>';
      pauseButton.setAttribute("disabled", "true");
      playButton.removeAttribute("disabled");
      return;
    }
